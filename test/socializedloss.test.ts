...
async function testSocializedLoss() {
  ...
  console.log(
    'Liqee Value',
    entropyAccount.getAssetsVal(entropyGroup, cache, 'Init').toString(),
  );
  ...
  console.log(
    'Liqee Health:',
    entropyAccount.getHealth(entropyGroup, cache, 'Maint').toString(),
  );
  ...
  if (entropyAccount.isBankrupt) {
    ...
    await client.resolveTokenBankruptcy(
      ...
      I80F48.fromNumber(
        Math.abs(
          entropyAccount.getNativeBorrow(liabRootBank, QUOTE_INDEX).toNumber(),
        ),
      ),
    );
  } else {
