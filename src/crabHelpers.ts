...
export async function getCrabDelta(symbol: string, pubkey: string=CRAB_PUBKEY, rpc_endpoint?: string) {
    ...
    const connection = new Connection(rpc_endpoint || config.cluster_urls[groupConfig.cluster]);
